...
table.clean %>% select(number_organism_name) %>% filter(if_any(everything(), ~str_detect(., "bacterium S5")))
...
```{r}
# Store hits found
write.table(x = best_tax_prok_tax_long, file = paste0(folder_path, "/data/federica_found_taxonomy_prok_tax_long.txt"), quote = F, sep = "\t", row.names = F)
...
write.table(x = best_tax_euk_tax_long, file = paste0(folder_path, "/data/federica_found_taxonomy_euk_tax_long.txt"), quote = F, sep = "\t", row.names = F)
...
## Read hits found. Avoid running all steps again.
# best_tax_euk_tax_long <- read_delim(file = paste0(folder_path, "/federica_found_taxonomy_euk_tax_short.txt"))
...
number_of_entries_euk_genus <- best_tax_euk_genus %>% count(taxonomy, sort = T )
 ...
 best_tax_euk_tax_long %>% filter(taxonomy_long == "Homo sapiens")
